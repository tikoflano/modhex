 ...
 * Origin: https://github.com/Yubico/php-yubico/blob/master/example/Modhex.php
 *
 */
...
class ModHex {
    ...
    static function modhexToB64($modhex_str) {
        $s = ModHex::Decode($modhex_str);
        return base64_encode($s);
    }

    static function b64ToModhex($b64_str) {
        $s = base64_decode($b64_str);
        return ModHex::Encode($s);
    }

    static function zeropad($num) {
        return (strlen($num) == 1) ? '0' . $num : $num;
    }

    static function b64ToHex($b64_str) {
        $s = '';
    ...
    static function hexToB64($hex_str) {
        $s = '';
